...
// Struct player -- Associe os dados de cada player
struct Player {
	Point initial_position;		// Posição inicial
	Point velocity;				// Velocidade do player
};

// Struct object -- associe todos os objetos que serão desenhados
struct Object {
	Point position;
	bool alpha;
};
...
int time() {
	...
	// struct player
    player p0; 	// Player Principal
    player e0;	// Player Inimigo
    


	// struct object
    object o0; 	// Maquina do tempo
    object o1;	// Espinho vertical
    object o2;	// Espinho horizontal


    // Seta jogadores
    setPlayer(&p0, 10, 10, 10, 10);
    //setPlayer(&e0, 10, 10, 10, 10);

    // Seta objetos
    setObject(&o0, 10, 10, true);
	//setObject(&o1, 10, 10, true);
	//setObject(&o2, 10, 10, true);
   ...
   external_load_map(&screen);
    ...
    SDL_UpdateWindowSurface(window);
        ...
        SDL_SetColorKey(tardis, SDL_TRUE, SDL_MapRGB(tardis->format, 0xFF, 0xFF, 0xFF));
        if (click[12] % 4 == 0) {
            if (offset.x > 375) {
                if (offset.y > 70) {
                    free(click);
